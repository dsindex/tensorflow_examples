...
"""A client that talks to mlp_mnist_inference service.

The client downloads test images of mnist data set, queries the service with
such test images to get classification, and calculates the inference error rate.
Please see mlp_mnist_inference.proto for details.

Typical usage example:

    mlp_mnist_client.py --num_tests=100 --server=localhost:9000
"""
...
def do_inference(hostport, work_dir, concurrency, num_tests):
  """Tests mlp_mnist_inference service with concurrent requests.

  Args:
    hostport: Host:port address of the mlp_mnist_inference service.
  ...
  stub = mlp_mnist_inference_pb2.beta_create_MLPMnistService_stub(channel)
  ...
  for _ in range(num_tests):
    request = mlp_mnist_inference_pb2.MLPMnistRequest()
