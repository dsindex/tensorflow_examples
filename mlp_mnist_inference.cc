...
// regression network trained and exported by mlp_mnist_export.py.
...
#include "tensorflow_serving/example/mlp_mnist_inference.grpc.pb.h"
#include "tensorflow_serving/example/mlp_mnist_inference.pb.h"
...
using tensorflow::serving::MLPMnistRequest;
using tensorflow::serving::MLPMnistResponse;
using tensorflow::serving::MLPMnistService;
...
class MLPMnistServiceImpl final : public MLPMnistService::Service {
 public:
  explicit MLPMnistServiceImpl(std::unique_ptr<SessionBundle> bundle)
      : bundle_(std::move(bundle)) {
    signature_status_ = tensorflow::serving::GetClassificationSignature(
        bundle_->meta_graph_def, &signature_);
  }

  Status Classify(ServerContext* context, const MLPMnistRequest* request,
                  MLPMnistResponse* response) override {
    ...
    // See mlp_mnist_export.py for details.
...
void RunServer(int port, std::unique_ptr<SessionBundle> bundle) {
  ...
  MLPMnistServiceImpl service(std::move(bundle));
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    LOG(FATAL) << "Usage: mlp_mnist_inference --port=9000 /path/to/export";
  }
  ...
  batching_parameters->mutable_thread_pool_name()->set_value(
      "mlp_mnist_service_batch_threads");
